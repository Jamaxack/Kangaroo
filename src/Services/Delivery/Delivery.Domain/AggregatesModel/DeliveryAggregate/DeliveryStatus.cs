...
namespace Delivery.Domain.AggregatesModel.DeliveryAggregate
{
    public class DeliveryStatus : Enumeration
    {
        ...
        public static IEnumerable<DeliveryStatus> List()
        {
            return new[]
            {
                New, Available, CourierAssigned, CourierDeparted, CourierPickedUp, CourierArrived, Completed,
                Reactivated, Canceled, Delayed, Failed
            };
        }
        ...
        public static DeliveryStatus FromName(string name)
        {
            ...
            if (state == null)
                throw new DeliveryDomainException(
                    $"Possible values for DeliveryStatus: {string.Join(",", List().Select(s => s.Name))}");
