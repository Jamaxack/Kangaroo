...
WORKDIR "/source/src/Services/Courier/Courier.API"
RUN dotnet build "Courier.API.csproj" -c Release

FROM build AS publish
RUN dotnet publish "Courier.API.csproj" -c Release -o /app/publish --no-build

# this layer is used for running unit tests in Continuous Integration
FROM build AS unittest
WORKDIR "/source/src/Services/Courier/Courier.UnitTests"
# on running this build target it will run the unit tests
CMD ["dotnet", "test", "--logger:trx"]
 
FROM build AS functionaltest
WORKDIR "/source/src/Services/Courier/Courier.FunctionalTests"
CMD ["dotnet", "test", "--logger:trx"]
